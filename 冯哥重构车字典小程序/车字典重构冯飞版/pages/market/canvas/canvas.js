...
Page({
  ...
  drawCanvas: function (vehicle) {
    ...
    //     ctx.draw(true)
        ...
        ctx.fillText('【出厂日期】', 5, 288)
        ctx.fillText('【上牌日期】', 5, 318)
        ctx.fillText('【排    量】', 5, 348)
        ctx.fillText('【真实公里】', 5, 378)
        ctx.fillText('【车源详情】', 5, 408)
        ctx.fillText('【过户次数】', 5, 438)
        ctx.fillText('【车辆所在地】', 5, 468)
        ctx.fillText('【联系方式】', 5, 498)
        ctx.fillText('【车辆性质】', 5, 528)
        ctx.fillText('【详细车况】', 5, 578)

        ctx.fillText(vehicle.dateOfManufacture, 120, 288)
        ctx.fillText(vehicle.licenseDate, 120, 318)
        ctx.fillText(vehicle.cc, 120, 348)
        ctx.fillText(vehicle.mileage+'万', 120, 378)
        if (vehicle.description.length > 20) {
          vehicle.description = vehicle.description.substring(0, 20) + '...';
        }
        ctx.fillText(vehicle.description, 120, 408)
        ctx.fillText(vehicle.transferTimes, 120, 438)
        ctx.fillText(vehicle.licenseLocation, 120, 468)
        ctx.fillText(vehicle.cellphoneNumber, 120, 498)
        if (vehicle.property==1){
          vehicle.property = '非营运'
        } else if (vehicle.property == 0){
          vehicle.property = '营运'
        }else{
          vehicle.property = ''
        }
        
        ctx.fillText(vehicle.property, 120, 528)
        ctx.setFontSize(12)
        ctx.setFillStyle('#999999')
        ctx.fillText('认真阅读群规，有序开展业务。', 180, 568)
        ctx.fillText('模版技术支持“二手车市场”小程序', 180,588)

        if (that.data.flag) {
          var str = 'https://www.chejiwei.com?openId=' + app.globalData.openid + '#' + vehicle.id;
          QR.api.draw(str, 'myCanvas', 120, 550, 48, 48)
        } else {
          //用户的二维码
          wx.downloadFile({
            url: that.data.createQRCode, //仅为示例，并非真实的资源
            success: function (res) {
              ctx.drawImage(res.tempFilePath, 120, 550, 48, 48)
              ctx.draw(true)
            }
          })
        }
