...
Page({
    ...
    url.ajaxPost(false, url.vehicle_detail, {
      ...
      if (options.id_) {
        wx.request({
          ...
          data: { promoterOpenId: promoterOpenId, openId: data.openId },
      ...
      if (typeof (options.q) != 'undefined') {
        ...
        if (paramArr.length == 2) {
          ...
          wx.request({
            url: url.agent_binding,
            data: { promoterOpenId: params[0], openId: data.openId },
            success: function (data) {
              console.log("分享绑定代理商:", data);
            }
          })
  ...
  callbackData: function (data) {
    ...
    data.result.vehicle.peizhi = '上牌日期:' + data.result.vehicle.licenseDate + '公里数:' + data.result.vehicle.mileage + '车辆所在地:' + data.result.vehicle.location + '牌照辖区:' + data.result.vehicle.licenseLocation + '出厂日期:' + data.result.vehicle.dateOfManufacture + '过户次数:' + data.result.vehicle.transferTimes + '排量:' + data.result.vehicle.cc + 'vin:' + data.result.vehicle.vin;
