...
Page({
  ...
  data: {
    ...
    share_path: 'pages/order/order-detail-baodan2/order-detail',
  },
  ...
  callback: function (car_data) {
    ...
    car_data.result.userinfo.licenseno = car_data.result.userinfo.licenseno.toUpperCase();
    if (car_data.result.userinfo.prooftype ==0){
      car_data.result.userinfo.prooftype='-';
    }else{
      car_data.result.userinfo.prooftype = code.prooftype [car_data.result.userinfo.prooftype]
    }
    if (car_data.result.userinfo.licensecolor == 0) {
      car_data.result.userinfo.licensecolor = '';
    } else {
      car_data.result.userinfo.licensecolor = code.licensecolor[car_data.result.userinfo.licensecolor]
    }
    if (car_data.result.userinfo.clausetype == 0){
      car_data.result.userinfo.clausetype = '-'
    }else{
      car_data.result.userinfo.clausetype = code.clausetype[car_data.result.userinfo.clausetype];
    }
    if (car_data.result.userinfo.runregion == 0){
      car_data.result.userinfo.runregion = '-'
    }else{
    car_data.result.userinfo.runregion = code.runregion[car_data.result.userinfo.runregion];
    }
    // 隐藏用户信息
    car_data.result.userinfo.licenseowner = util.encryptionStr(car_data.result.userinfo.licenseowner, 2);
